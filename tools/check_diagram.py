#!/usr/bin/env python3
"""Deterministic structural checker for animated SVG diagrams.

Adapted from dashmotion's check_diagram.py (csthink/dashmotion, MIT).
Runs the original C1..C9 + parse checks. Accepts .svg or .html files:
it greps the <svg>..</svg> block out of whatever input you give it.
...
Failure classes:
  ...
  C3 dash-seam      |stroke-dashoffset delta| is not a multiple of dasharray period
...
Not checked (documented limitation): text/label collisions (needs font metrics),
boundary 20px padding, legend placement.

Usage: python tools/check_diagram.py file1 [file2 ...] [--json]
Exit code 0 always; results go to stdout. A clean pass prints 0 violations.
"""
import json
import math
import re
...
def extract_svg(html_or_svg):
    if "<svg" in html_or_svg:
        m = re.search(r"<svg\b[\s\S]*?</svg>", html_or_svg)
        if not m:
            return None
        svg = m.group(0)
    else:
...
def check_file(path):
    ...
    classes, keyframes = parse_css(raw)
    nodes_all = []
    L = []
    walk(root, [], L)
    nodes_all = L
